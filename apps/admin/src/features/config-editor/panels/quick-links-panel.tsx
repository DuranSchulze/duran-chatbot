...
export function QuickLinksPanel({ quickLinks, onChange }: QuickLinksPanelProps) {
  const [draft, setDraft] = useState<Partial<QuickLink>>({ label: "", url: "", icon: "link" })

  const updateLink = (id: string, updates: Partial<QuickLink>) => {
    onChange(quickLinks.map((link) => (link.id === id ? { ...link, ...updates } : link)))
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
          ...
          <Card className="border-dashed bg-slate-50/70 shadow-none">
            <CardContent className="flex flex-col items-center justify-center gap-3 py-10 text-center">
              ...
              <div className="space-y-1">
                <h3 className="text-base font-semibold text-foreground">No quick links yet</h3>
                <p className="max-w-md text-sm leading-6 text-muted-foreground">Add booking, contact, or support shortcuts here.</p>
              </div>
            </CardContent>
          </Card>
        ) : null}
        ...
        {quickLinks.map((link) => (
          <Card key={link.id} className="border-border bg-white shadow-none">
            <CardContent className="space-y-4 pt-5">
              ...
              <FieldGrid>
                ...
                <Field>
                  ...
                  <Select
                    value={link.icon}
                    onChange={(event) => updateLink(link.id, { icon: event.target.value })}
                  >
      ...
      <Card className="border-dashed bg-slate-50/70 shadow-none">
        <CardContent className="space-y-4 pt-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-foreground">Add a quick link</h3>
              <FieldDescription>Create a new shortcut for the widget menu.</FieldDescription>
            </div>
          </div>
          <FieldGrid>
            <Field>
