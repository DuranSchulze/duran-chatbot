          ...
          <Card className="border-dashed bg-slate-50/70 shadow-none">
            <CardContent className="flex flex-col items-center justify-center gap-3 py-10 text-center">
              ...
              <div className="space-y-1">
                <h3 className="text-base font-semibold text-foreground">No knowledge entries yet</h3>
                <p className="max-w-md text-sm leading-6 text-muted-foreground">Add FAQs, policies, or trusted knowledge here.</p>
              </div>
            </CardContent>
          </Card>
        ) : null}
        ...
        {dataset.map((entry) => (
          <Card key={entry.id} className="bg-white shadow-none">
            <CardContent className="space-y-4 pt-5">
              ...
              <p className="rounded-2xl border border-border bg-slate-50 p-4 text-sm leading-7 text-muted-foreground">
                {entry.content}
              </p>
            </CardContent>
          </Card>
      ...
      <Card className="border-dashed bg-slate-50/70 shadow-none">
        <CardContent className="space-y-4 pt-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-foreground">Add dataset entry</h3>
              <FieldDescription>Add the content and keywords that should trigger it.</FieldDescription>
            </div>
          </div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(draft.keywords ?? []).map((item) => (
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-full border border-primary/15 bg-primary/10 px-3 py-1 text-xs font-medium text-primary"
                  >
                    {item}
                    <X className="size-3" />
                  </button>
          ...
          <Button
            onClick={addEntry}
            disabled={!draft.title || !draft.content || !(draft.keywords?.length ?? 0)}
          >
            <Plus className="size-4" />
            Add Entry
          </Button>
